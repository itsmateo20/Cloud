...
function getPasscodeHashWorkFactor() {
    const configuredValue = Number(process.env.SHARE_PASSCODE_HASH_ROUNDS || 12);
    if (!Number.isFinite(configuredValue) || configuredValue < 4) return 12;
    return Math.min(Math.floor(configuredValue), 15);
}

async function hashPasscode(passcode) {
    return bcrypt.hash(String(passcode), getPasscodeHashWorkFactor());
}
...
async function verifyPasscode(passcode, hash) {
    ...
    try {
        return await bcrypt.compare(String(passcode), String(hash));
    } catch {
        return false;
    }
}
...
export async function cleanupExpiredShares() {
    await prisma.$executeRawUnsafe(
        `DELETE FROM SharedCollection WHERE expiresAt IS NOT NULL AND datetime(expiresAt) <= datetime('now')`
    );
}
...
export async function createShare({ ownerId, name, requireLogin, allowedEmails, passcode, expiresAt, items }) {
    ...
    await prisma.$executeRawUnsafe(
        ...
        normalizedPasscode ? await hashPasscode(normalizedPasscode) : null,
        expiresAt || null
    );
...
export async function updateShareByOwner(ownerId, shareId, updates) {
    ...
    const nextPasscodeHash = updates.clearPasscode
        ? null
        : (normalizedPasscode ? await hashPasscode(normalizedPasscode) : current.passcodeHash || null);
...
export async function canAccessShare(share, session, passcode) {
    if (!share) {
    ...
    if (hasPasscode) {
        ...
        if (!(await verifyPasscode(normalizedPasscode, share.passcodeHash))) {
