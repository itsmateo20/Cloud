...
import { signAccountDeletionEmail } from "./accountDeletion";
    ...
    try {
        ...
        if (user.isDeleted) {
            return {
                ...
                email: user.email,
                signature: signAccountDeletionEmail(user.email),
...
export async function authenticationWithGoogle(email, type) {
    ...
    try {
        ...
        if (userByGoogleEmail) {
            if (userByGoogleEmail.isDeleted) {
                return {
                    success: false,
                    code: "account_deleted",
                    email: userByGoogleEmail.email || userByGoogleEmail.googleEmail,
                    signature: signAccountDeletionEmail(userByGoogleEmail.email || userByGoogleEmail.googleEmail),
                    deletionScheduledAt: userByGoogleEmail.deletionScheduledAt,
                };
            }
        ...
        if (userByEmail && userByEmail.isDeleted) {
            return {
                success: false,
                code: "account_deleted",
                email: userByEmail.email || userByEmail.googleEmail,
                signature: signAccountDeletionEmail(userByEmail.email || userByEmail.googleEmail),
                deletionScheduledAt: userByEmail.deletionScheduledAt,
            };
        }
