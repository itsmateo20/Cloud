...
function meetsPasswordRequirements(password) {
    return /[A-Z]/.test(password) &&     // Uppercase letter
...
export async function signIn(email, password) {
    try {
        ...
        if (!meetsPasswordRequirements(password)) {
            return { success: false, code: "password_requirements_false" };
        }
        ...
        return { success: false, code: "signin_error", error };
    }
}
...
export async function signUp(email, password) {
    try {
        ...
        return { success: false, code: "signup_error", error };
    }
}
...
export async function signUpWithThirdParty(email, password, type, signature) {
    try {
        ...
        console.error("Error in signUpWithThirdParty:", error);
        return { success: false, code: "signup_error", error };
    }
}
...
export async function authenticationWithGoogle(email, type) {
    if (!email) {
        return { success: false, code: "email_missing" };
    }
    ...
    try {
        ...
        if (userByGoogleEmail) {
            return { success: true, user: userByGoogleEmail };
        }

        if (type === "signup" && userByEmail) {
            if (!userByEmail.provider.includes("google")) {
                return { success: false, code: "user_already_exists_link_google" };
            } else {
                return { success: false, code: "user_already_exists_linked" };
            }
        }
        ...
        return { success: false, code: "authentication_error", error };
    }
}
...
export async function linkAccount(googleEmail, email, password, type) {
    try {
        ...
        return { success: false, code: "link_account_error", error };
    }
}
