...
function isSecureRequest(req) {
    const configured = String(process.env.AUTH_COOKIE_SECURE == "true" || "").trim().toLowerCase();
    ...
    try {
        return new URL(req?.url || "http://localhost").protocol === "https:";
    } catch {
...
export async function createSession(user, req = null) {
    ...
    if (!secret) return { success: false, code: "auth_secret_missing" };
...
export async function getSession() {
    ...
    try {
        ...
        if (userExists.isDeleted) {
            await revokeClientTokenRecord(user.id, user.tid).catch(() => { });
            cookieStore.delete("auth");
